...
export function PhongBanFormDialog({ open, mode, current, onClose }: Props): JSX.Element {
  ...
  function setField<K extends keyof PhongBanForm>(key: K, value: PhongBanForm[K]) {
  ...
  function handleSubmit(e: FormEvent) {
    ...
    if (mode === 'edit' && current) {
      update.mutate({ maPb: current.ma_pb, body: form }, { onSuccess: onClose, onError });
    } else {
  ...
  return (
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="sm">
          ...
          <Stack spacing={2} sx={{ mt: 0.5 }}>
            ...
            <Stack direction="row" spacing={2}>
              <TextField
                ...
                autoFocus={mode !== 'edit'}
                value={form.ma_pb}
                onChange={(e) => setField('ma_pb', e.target.value.toUpperCase())}
                sx={{ flex: 1 }}
                  ...
                  endAdornment: (
                    <InputAdornment position="end">
                      <Tooltip title="Chọn tài khoản">
                        <IconButton edge="end" size="small" onClick={() => setPickTk(true)}>
      ...
      <TaiKhoanPickerDialog
        ...
        onSelect={(r) => setForm((f) => ({ ...f, ma_td1: r.tk, ten_tk: r.ten_tk }))}
      />
    </Dialog>
  );
}
