...
/** Tra cứu module theo slug (khớp path /accounting/:moduleSlug ở client). */
...
/** Thứ tự hiển thị module (menu Kế toán). */
...
/**
 * Trang mặc định khi vào module Kế toán (module đầu menu). Khác fe_maxv: không có
 * :slug công ty trên URL — module Kế toán dùng chung công ty đang chọn của cả app
 * (useActiveCompanyMst), không tự quản tenant riêng.
 */
export function defaultModulePath(): string {
  return `/accounting/${MODULE_ORDER[0].slug}`;
}
