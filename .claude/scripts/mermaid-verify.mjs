...
function findChrome() {
  ...
  for (const versionDir of fs.readdirSync(glob)) {
    // Puppeteer đặt binary theo layout riêng mỗi HĐH/kiến trúc — thử lần lượt, lấy cái đầu tiên có thật.
    const candidates = [
    ...
    for (const parts of candidates) {
      const candidate = path.join(glob, versionDir, ...parts);
...
blocks.forEach((b, i) => {
  ...
  // Windows: mmdc là shim .cmd — Node >=20 từ chối spawn .cmd nếu không qua shell (EINVAL),
  // nên bật shell và tự bọc nháy kép cho path (tmpdir/PNG_DIR có thể chứa dấu cách).
  const isWin = process.platform === 'win32';
  const arg = (v) => (isWin ? `"${v}"` : v);
  const res = spawnSync('mmdc', ['-i', arg(mmdPath), '-o', arg(outPath), '-s', '2'], {
    encoding: 'utf8',
    shell: isWin,
