...
4. Nếu OQ inherited từ upstream → skill **update cả upstream doc** (mark `[x]`/`[~]` + thêm note "resolved via /{current_skill}"). L2 diff cho mỗi upstream edit. Ghi lại từng edit vào mục tiến độ của `docs/{feature}/CONTEXT_SUMMARY.md`.
...
Mỗi doc bị update (hiện tại + upstream + downstream) → 1 dòng riêng trong mục tiến độ của `docs/{feature}/CONTEXT_SUMMARY.md`:
...
Sau khi loop xong (resolve ≥1 OQ), ghi sự kiện cho doc hiện tại vào `docs/{feature}/CONTEXT_SUMMARY.md`. Note chuẩn:
