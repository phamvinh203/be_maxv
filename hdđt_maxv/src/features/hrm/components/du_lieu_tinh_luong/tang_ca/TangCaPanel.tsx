 ...
 *
 * ⚠️ `gio_nam` (cột "Giờ OT kỳ này" ở `DanhSachTangCaCard`, RVW-709 đổi nhãn từ
 * "Tổng giờ năm" — KHÔNG còn tô theo `nguong_vuot_muc_tc_nam` vì gây hiểu nhầm
 * đã kiểm tra trần năm) chỉ bằng giờ OT **của kỳ này** (`totalHours`) — API
...
export default function TangCaPanel() {
  ...
  // RVW-711: nhân viên có tăng ca trong kỳ nhưng bị 3 ô lọc ẩn khỏi bảng — dữ
  // liệu vẫn tính vào lương, chỉ là không ai thấy để kiểm tra ở màn này.
  const soNgoaiBoLoc = demSoNgoaiBoLoc(nhanVien, otData);
  ...
  return (
    <Stack spacing={2.5}>
      ...
      <ThanhCongCuBangNhap
        ...
        disabledApDung={isReadOnly || mau.length === 0 || rows.length === 0}
        ...
        nhanApDung="Áp dụng tăng ca"
        nhanQuanLy="Quản lý tăng ca"
        canhBaoChuaApDung="Bảng tăng ca có nội dung chưa áp dụng"
      />
      ...
      <DanhSachTangCaCard
        ...
        isReadOnly={isReadOnly}
      />
      <CanhBaoNgoaiBoLoc soLuong={soNgoaiBoLoc} module="tăng ca" />
