 ...
 * dùng chọn. BE trả 422 nếu mã sai/thiếu captcha, 501 nếu NCC chưa hỗ trợ — `apiFetchBlob` ném
 * `ApiError` kèm message để nơi gọi đếm lỗi từng hóa đơn.
 *
 * Dùng: `DownloadOriginalDialog` (nút "Tải xuống").
 */
export function taiHoaDonGoc({ msttcgp, code, sellerMst, recaptcha }: TaiHoaDonGocParams): Promise<Blob> {
