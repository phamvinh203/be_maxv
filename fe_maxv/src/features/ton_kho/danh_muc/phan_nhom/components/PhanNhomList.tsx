...
export function PhanNhomList(): JSX.Element {
  const { data, isLoading, isFetching, isError, error, refetch } = usePhanNhomList();
  ...
  const [form, setForm] = useState<{ open: boolean; mode: PhanNhomMode; current: PhanNhom | null }>({
    open: false,
    mode: 'new',
    current: null,
  });
            ...
            {!isLoading && list.filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={5} align="center" sx={{ py: 6, color: 'text.secondary' }}>
                  {list.searchInput ? 'Không tìm thấy nhóm phù hợp' : 'Chưa có nhóm nào'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
