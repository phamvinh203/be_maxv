...
| OQ-tkt-05 `[MỚI 2026-09-14 — phát sinh từ phản biện Architect P-12]` | 3 chỉ tiêu `ct24`/`ct25`/`ct32` của mẫu 05/KK-TNCN chưa xác nhận được định nghĩa pháp lý chính xác (code nháp cũ để `0` cho cả 3; `ct32` đã có đề xuất công thức tạm ở `data-model-to-khai-thue.md` Mục 8) | Kế toán trưởng, đối chiếu Thông tư 89/2026/TT-BTC bản gốc | Nếu để `0`/công thức tạm, kế toán phải ghi đè tay 3 chỉ tiêu này mỗi quý — đi ngược mục tiêu G3 |
