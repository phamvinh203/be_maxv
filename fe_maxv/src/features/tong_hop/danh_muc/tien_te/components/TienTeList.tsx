...
import type { TienTe } from '@/features/tong_hop/danh_muc/tien_te/types';
import { TienTeFormDialog, type TienTeMode } from './TienTeFormDialog';

const SEARCH_KEYS = ['ma_nt', 'ten_nt'];

export function TienTeList(): JSX.Element {
  const { data, isLoading, isFetching, isError, error, refetch } = useTienTeList();
  const del = useDeleteTienTe();

  const rows = useMemo(() => data ?? [], [data]);
  const list = useCatalogList<TienTe>({
    rows,
    getId: (r) => r.ma_nt,
  ...
  const [form, setForm] = useState<{ open: boolean; mode: TienTeMode; current: TienTe | null }>({
    open: false,
    mode: 'new',
    current: null,
  });
  ...
  const openForm = (mode: TienTeMode, current: TienTe | null) =>
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%', bgcolor: 'background.default' }}>
      <CatalogToolbar
        addLabel="Thêm ngoại tệ"
        onAdd={() => openForm('new', null)}
        searchValue={list.searchInput}
        onSearchChange={list.setSearchInput}
        searchPlaceholder="Tìm mã / tên ngoại tệ…"
      ...
      <Stack direction="row" sx={{ alignItems: 'center', px: 2, py: 0.5, gap: 1 }}>
        ...
        <Typography variant="caption" color="text.secondary" sx={{ ml: 'auto' }}>
          {isLoading ? 'đang tải…' : `${list.filtered.length} ngoại tệ`}
        ...
        <Table size="small" stickyHeader>
          <TableHead>
            <TableRow>
              ...
              <TableCell>Tên ngoại tệ</TableCell>
              <TableCell>Tên khác</TableCell>
              <TableCell align="right">Số TP</TableCell>
          ...
          <TableBody>
            ...
            {list.paged.map((r) => (
              ...
              >
                <TableCell sx={{ fontWeight: 600 }}>{r.ma_nt}</TableCell>
                <TableCell>{r.ten_nt}</TableCell>
                <TableCell>{r.ten_nt2 || '—'}</TableCell>
                <TableCell align="right">{r.ra_ndec}</TableCell>
            ...
            {!isLoading && list.filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={5} align="center" sx={{ py: 6, color: 'text.secondary' }}>
                  {list.searchInput ? 'Không tìm thấy ngoại tệ phù hợp' : 'Chưa có ngoại tệ nào'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
      ...
      <DeleteDialog
        ...
        message={
          selected
            ? `Bạn có chắc chắn muốn xóa ngoại tệ "${selected.ma_nt} - ${selected.ten_nt}"? Hành động này không thể hoàn tác.`
            : ''
        }
