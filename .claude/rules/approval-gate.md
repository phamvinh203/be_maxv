...
Lý do bỏ mô hình "sub-agent Write thẳng rồi rollback nếu user từ chối": rollback không đáng tin. `git checkout -- file` trả về HEAD (không phải trạng thái ngay trước sub-agent) → nuốt thay đổi chưa commit; `rm` không phân biệt file vừa tạo với file untracked có sẵn; hook PostToolUse đã kịp chạy side-effect không hoàn tác được. An toàn dữ liệu > tốc độ song song.
