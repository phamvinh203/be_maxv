...
* **Trạng thái phân hệ (cập nhật 2026-09-08 sau Quality Gate và vòng sửa lỗi chặn — xem Mục 16):**
  ...
  * **Màn Chốt kỳ lương (2026-09-11) — xem Mục 22:** chủ dự án yêu cầu trực tiếp và chốt 4 quyết định nghiệp vụ (`BR-dltl-030…034`). Đã triển khai máy chủ + giao diện, test máy chủ đạt, DB dev/local đã có bảng mới. **Chưa qua** Tester-QA Phase B / Code Reviewer độc lập, **chưa** kiểm giao diện trên trình duyệt đã đăng nhập, production **chưa** `sync:tenants`.
  * **Sub-cụm Tờ khai thuế TNCN (`to_khai_thue`, 2026-09-14) — bắt đầu Phase 1 Discovery:** business-analyst nghiên cứu (4 agent song song: khảo sát codebase + 3 hướng luật thuế) phát hiện khung pháp lý thuế TNCN đã đổi toàn diện từ kỳ tính thuế 2026 (Luật 109/2025/QH15, NĐ 253/2026/NĐ-CP, NQ 110/2025/UBTVQH15, Thông tư 87/2026 và 89/2026/TT-BTC) — biểu thuế 7 bậc/giảm trừ 11tr-4,4tr mà quyết định 2026-09-10 (mục trên) từng chốt giữ nguyên nay xác nhận là **lỗi thời** (quyết định đó dựa trên hiểu biết luật cũ tại thời điểm chốt). Đã xuất `docs/hrm/to_khai_thue/brainstorms/2026-09-14-to-khai-thue-tncn-brainstorm.md` (3 tính năng: thu-nhap-ngoai-luong, bang-tinh-thue, to-khai-tncn; kiến trúc biểu thuế/giảm trừ cấu hình theo mốc hiệu lực — effective-dated, thay singleton hiện tại) và **sửa `srs/hrm-spec.md`** (BR-hrm-081, AC-hrm-67, FR-hrm-045/047/055, A-hrm-12, giá trị mặc định `personalDeduction`/`dependentDeduction`/`taxBrackets`, đóng `OQ-hrm-35` — xem Mục 15.2b) để khớp luật mới. **Việc CÒN NỢ, ngoài phạm vi 3 tính năng**: công ty đã remediate theo FR-hrm-055 (đang ở biểu 7 bậc, tưởng đúng) nay cần một đợt chuẩn hóa tiếp theo sang biểu 5 bậc luật 2025 — Architect/Backend đánh giá khi triển khai. **Cập nhật cùng ngày — SRS chính thức xong:** `docs/hrm/to_khai_thue/srs-to-khai-thue.md` (prefix ID `tkt`, 18 BR/18 FR/21 mã lỗi/8 UC/28 AC, 4 thực thể mới đánh dấu `[ĐỀ XUẤT]` chờ Architect xác nhận). **Phát hiện thêm khi viết SRS (SRS Mục 14 điểm 5)**: `docs/hrm/du_lieu_tinh_luong/srs-du-lieu-tinh-luong.md` (BR-dltl-025/026/027 — đã ký duyệt + kích hoạt Backend Engineer từ 2026-09-10) cũng đang dùng số liệu thuế TNCN đã hết hiệu lực (miễn OT một phần thay vì 100%, ngưỡng khấu trừ 10% = 2tr thay vì 5tr, trần miễn thuế ăn trưa 730k thay vì 1,2tr) — đã tạo task riêng (`task_85b3ae1d`) để xử lý, CHƯA sửa trong đợt `to_khai_thue`.

**Phase 2 (Architect ∥ Tester-QA) hoàn tất cùng ngày 2026-09-14.** Architect bàn giao `docs/hrm/to_khai_thue/{data-model,api-contract}-to-khai-thue.md` (5 bảng mới: `hrm_tax_policies`, `hrm_other_income_categories`, `hrm_other_income_records` mở rộng, `hrm_tax_calculation_lines`, `hrm_to_khai_tncn05` mở rộng; 22 endpoint REST) + `docs/hrm/architecture/adr/ADR-012` (biểu thuế/giảm trừ theo mốc hiệu lực, đóng `OQ-hrm-35`) + `ADR-013` (snapshot 2 tầng bất biến số thuế, tái dùng `hrm_payroll_module_locks` + enum `TAX_SHEET` mới). **Phát hiện quan trọng nhất (data-model Mục 0)**: đã tồn tại sẵn code nháp CHƯA COMMIT (schema + 8 file BE + FE) viết theo luật cũ (TT80/2021, ngưỡng 2tr, kỳ tháng/quý lẫn lộn) — đây là việc sửa nháp lệch luật, không phải xây mới. Tester-QA bàn giao `test-matrix`/`test-cases-to-khai-thue.md` (126 test case, 13 nhóm).

**BA Final Sign-off hoàn tất cùng ngày.** Đối soát 17 phản biện Architect + 12 gap QA, xử lý dứt điểm 16/17 điểm (đã cập nhật vào SRS): chốt hợp đồng khoán đi nhánh lũy tiến (đóng luôn `EC-dltl-07` bên `du_lieu_tinh_luong`), bỏ xuất XML (giữ Excel+PDF), đưa BH hưu trí tự nguyện/từ thiện ra ngoài phạm vi (sửa AC-tkt-016), chuyển danh mục "ăn ca DN tự nấu" sang nhóm miễn 100%, làm rõ trần năm tính lũy kế, mở rộng mã lỗi Cam kết 08, bổ sung 7 mã lỗi kỹ thuật (`E-tkt-015…021`), thêm yêu cầu thông tin người nộp thuế vào FR-tkt-014, thêm quy tắc dọn dữ liệu treo khi mở lại tháng, thêm `OQ-tkt-05` (3 chỉ tiêu mẫu tờ khai chưa rõ định nghĩa). **1 điểm còn treo thật sự**: `GAP-QA-tkt-01` (thu nhập chịu thuế toàn phần trả theo NET — công thức quy đổi phụ thuộc vòng vào thuế suất lũy tiến biên) — cần quyết định trước khi Backend code nhánh đó.

**⚠️ 2 việc CHẶN triển khai thật sự, KHÔNG thuộc phạm vi `to_khai_thue`, phát hiện qua Architect review:**
1. ~~`task_defff3dd` (mức nghiêm trọng CAO NHẤT)~~ — **ĐÃ XỬ LÝ 2026-09-14 19:29** (`work-log.md`, BUG-dltl-003): `tinhThueLuyTien()` nay nhận biểu thuế làm tham số và đọc `GeneralSetting.taxBrackets`; biểu chuẩn đổi sang 5 bậc Luật 109/2025/QH15, giảm trừ 15.5tr/6.2tr; script `FR-hrm-055` mở rộng để nhận diện cả biểu 7 bậc cũ + 2 mức giảm trừ cũ và đã chạy thật cho tenant duy nhất (`maxv_0106861880_app`), chạy lại lần 2 không ghi thêm (AC-hrm-71 pass). Tenant này có 0 kỳ lương đã chốt nên không có số liệu lịch sử bị ảnh hưởng. Tra chính sách theo mốc hiệu lực (ADR-012) **đã có từ bước 1+2** (`hrm_tax_policies`, commit `c0ab93c`) và đang dùng cho Bảng tính thuế; engine Bảng lương vẫn đọc `GeneralSetting` singleton. `OQ-tkt-02` **đã chốt**: áp biểu mới cho toàn bộ năm 2026.
2. ~~`task_85b3ae1d`: BR-dltl-025/026/027 dùng ngưỡng/tỷ lệ cũ~~ — **ĐÃ XỬ LÝ 2026-09-14 19:50** (`work-log.md`, BUG-dltl-004): tiền làm thêm giờ nay miễn thuế **toàn bộ** (thay vì chỉ phần chênh lệch), ngưỡng khấu trừ 10% **2tr → 5tr**, trần miễn thuế ăn ca **730k → 1,2tr**; đã sửa đồng bộ mặc định `schema.prisma` + `khoiTaoCauHinhMacDinh()` + engine + SRS `du_lieu_tinh_luong` + data-model/api-contract/dev-notes, và chạy script chuẩn hóa cho tenant duy nhất. **Còn nợ**: `test-matrix-bang-luong-tong-hop.md` (~27 chỗ số cũ) chờ tester-qa rà cùng Phase B — bộ ca thực thi trong `src/__tests__/` đã cập nhật đủ.

**Đối soát mã nháp với đặc tả đã ký (2026-09-14, sau khi đóng 2 việc chặn):** xem `docs/hrm/to_khai_thue/doi-soat-ma-nhap-to-khai-thue.md`. Kết luận: backend **viết lại phần lõi** (3 service xương sống + 3 file biên), `types.ts` sửa, `xuatXmlTncn05.ts` xóa; tái dùng ~15–20%, trong đó **bắt buộc giữ** 9 luật cân đối tờ khai và bộ công thức `ct16`–`ct31`. Frontend giữ ~55–60% khung (bảng tính thuế ~70%). Lý do viết lại: mã nháp thiếu 4 khái niệm TRUNG TÂM của bản ký (danh mục 4 nhóm xử lý thuế · snapshot kết quả · chính sách thuế theo mốc hiệu lực · snapshot tháng + vòng đời 4 trạng thái) — **ADR-012 và ADR-013 đều ở mức 0%**. Tài liệu có sẵn thứ tự 8 bước cho phase backend-engineer, bắt đầu từ di trú M-1/M-2 chứ không từ endpoint.

> **Lưu ý về `prisma/tenant/schema.prisma`:** commit `f473fc6` có kèm 1 enum + 2 model nháp của `to_khai_thue` (`TaxDeductionType`, `OtherIncomeRecord`, `hrm_to_khai_tncn05`) — đây đúng là **trạng thái xuất phát** mà `data-model-to-khai-thue.md` **Mục 0** đã ghi nhận và thiết kế dựa trên, kèm 6 điểm lệch L-1…L-6 và lộ trình di trú M-1…M-4 xây trên chính 2 bảng đó. **Không phải thiết kế đã duyệt** — chưa chạy `sync:tenants`, và phải sửa theo Mục 0 trước khi đẩy xuống tenant. *(Đã xử lý ở bước 0 — commit `cc451db`: sửa theo Mục 0 rồi mới đồng bộ tenant.)*

**Status tổng thể sub-cụm `to_khai_thue`: Đặc tả + thiết kế kiến trúc + test design ĐÃ CHỐT đầy đủ (BA Final Sign-off xong). Cả 2 việc chặn ĐÃ XỬ LÝ XONG 2026-09-14** (BUG-dltl-003 + BUG-dltl-004) — engine `payrollCalculation.service.ts` mà `to_khai_thue` tái dùng (A-tkt-03) nay đã tính đúng luật 2026 và đọc đủ tham số từ cấu hình. **Trước khi bật cờ `Ready for Implementation` còn 2 điểm cần chủ dự án/BA quyết:** (a) `GAP-QA-tkt-01` — công thức quy đổi thu nhập NET chịu thuế toàn phần (phụ thuộc vòng vào thuế suất biên), cần chốt trước khi Backend code nhánh đó; (b) `OQ-tkt-02` — có phải tính lại kỳ T1–T6/2026 theo biểu cũ hay không; nếu CÓ thì phải làm phần hiệu lực theo mốc thời gian của ADR-012 trước, nếu KHÔNG thì hiện trạng đã đúng. Hiện chưa kỳ lương nào được chốt nên (b) chưa gây rủi ro số liệu.

**Tiến độ backend-engineer `to_khai_thue` (cập nhật 2026-09-15):** `OQ-tkt-02` đã chốt — áp biểu mới cho cả năm 2026 (dòng chính sách mốc 2026-01-01 + dòng luật cũ mốc 1900-01-01). Xong cả 8 bước (0–7) của lộ trình trong `doi-soat-ma-nhap-to-khai-thue.md`, chi tiết từng bước ở `work-log.md`:

| Bước | Nội dung | Commit |
|---|---|---|
| 0 | Di trú M-1/M-2 (4 enum, 3 bảng mới, mở rộng bản ghi thu nhập ngoài lương) + seed chính sách thuế và 12 danh mục | `cc451db` |
| 1+2 | ADR-012 tra chính sách theo mốc hiệu lực + hạ tầng 21 mã lỗi `E-tkt` | `c0ab93c` |
| 3 | Danh mục loại thu nhập ngoài lương (5 endpoint) | `2b6dfd3` |
| 4 | Bản ghi thu nhập ngoài lương + engine 4 nhóm xử lý thuế (6 endpoint) | `286284e` |
| 5 | Bảng tính thuế tháng: xem 2 nguồn / chốt / mở lại (3 endpoint), ADR-013 tầng tháng | `6c38845` |
| 6 | Tờ khai quý 05/KK-TNCN: xem (tự tạo khi đủ 3 tháng) · lịch sử kỳ · ghi đè · xuất Excel/PDF · tải lại file · bảng chi tiết · đánh dấu đã nộp (8 endpoint) + cổng quyền `E-tkt-014` cho cả sub-cụm | `e7922ee` · `3ff4232` |
| 7 | Dọn + nối route chính thức: 23 endpoint chạy từ file route đã commit, bỏ 8 route nháp, cất ~2.060 dòng mã nháp cũ vào git stash, bổ sung `dangApDung` cho danh sách chính sách thuế, test HTTP giả lập cho ma trận quyền | `11e2ab1` |

**Quyết định chủ dự án 2026-09-15 — "tách 2 phần" thu nhập ngoài lương:** khoản khấu trừ riêng (`WITHHOLDING_FLAT`) vẫn nằm trong thu nhập chịu thuế của dòng (để tờ khai quý báo đủ thu nhập) nhưng KHÔNG vào nền lũy tiến. Thêm cột `thu_nhap_khau_tru_rieng`, đã đồng bộ xuống tenant (chỉ thêm cột, không mất dữ liệu). Đã cập nhật SRS Mục 4.3 + BR-tkt-007/012, data-model Mục 3.4/5.1/5.3, api-contract Mục 4.

**Còn chờ quyết định:** `GAP-QA-tkt-01` (khoản chịu thuế toàn phần trả NET — hiện đang chặn bằng E-tkt-004) · mở rộng BR-tkt-003 cho cả 4 nhóm · `OQ-tkt-03` · `OQ-tkt-05` · 2 lỗ hổng đặc tả phát hiện ở bước 5: người thời vụ/thử việc có khoản chịu thuế toàn phần, và người không có hợp đồng hiệu lực nhận thưởng — cả hai hiện KHÔNG phát sinh thuế trên khoản đó. **Ngoài phạm vi (lúc đó):** engine lương đếm người phụ thuộc không xét kỳ đăng ký — đã tách task riêng; **nay đã sửa** trong đợt sửa lỗi Phase B (ISSUE-tkt-001, xem đoạn "Sửa lỗi Phase B" bên dưới). Cả 23 endpoint đã nối route và commit (`11e2ab1`). Mã nháp cũ phía máy chủ nằm trong git stash có nhãn `to_khai_thue: ma nhap cu da viet lai o buoc 3-6` (không commit); giao diện nháp `hdđt_maxv` vẫn gọi các route cũ đã bỏ — sửa khi bật lại frontend.

**Tester-QA Phase B `to_khai_thue` (2026-09-15):** bộ kiểm thử HTTP thật trên 2 DB tenant tự cấp rồi xóa (`be_maxv/src/__tests__/hrm/hrmToKhaiThueApi.test.ts`, commit `9fd34ec`) — 133 ca lá: **113 đạt · 7 lỗi · 13 bỏ qua có lý do**; theo mã bộ ca: 110/126 đạt qua API, 4 mã lỗi (TC-tkt-015, 021, 047, 117). Bộ chỉ tiêu Q3 tính tay khớp 17/17; chốt/mở lại tháng, ghi đè, xuất Excel, tải lại file, đánh dấu nộp, 3 ca đua dữ liệu và cô lập tenant đều đạt. Hồi quy toàn bộ: 1160 test, 1132 đạt, 15 lỗi = 4 lỗi cũ (TC-hrm-301/316 và 2 nhóm cha) + 11 của bộ mới — không hồi quy chỗ khác. **Lỗi cần backend sửa:** BUG-tkt-001 (mã nhân viên không tồn tại trả 409 thông điệp chung, không có mã, thay vì 404 E-tkt-016) · BUG-tkt-002 (danh mục khấu trừ 100% + trả NET: tính thử trả số rỗng, ghi thật lỗi 500) · BUG-tkt-003 (cá nhân không cư trú: nhãn khấu trừ 20% nhưng tính 10% — cần BA chọn hướng sửa). **Cần BA/chủ dự án quyết:** ISSUE-tkt-001 engine lương đếm người phụ thuộc không theo kỳ đăng ký nên thuế trên bảng lương và bảng tính thuế lệch nhau (ví dụ T8: 165.000 so với 475.000) · ISSUE-tkt-002 người không có thu nhập trong kỳ vẫn lên bảng thuế và được đếm vào chỉ tiêu [16]/[17]. Chi tiết: `to_khai_thue/test-report-to-khai-thue.md` và `to_khai_thue/issues-and-bugs-to-khai-thue.md`. **Bước tiếp theo:** backend-engineer sửa BUG-tkt-001/002 (BUG-tkt-003 sau khi BA chọn hướng) → chạy lại Phase B → code-reviewer.

**Sửa lỗi Phase B + chạy lại (2026-09-15, commit `9fd34ec`):** chủ dự án chốt 3 quyết định — chặn cá nhân không cư trú cho tới khi làm nhánh khấu trừ 20% (BUG-tkt-003) · chỉ tiêu [16]/[17] không đếm người không được trả đồng nào trong quý, Bảng tính thuế tháng vẫn giữ dòng 0 đồng (ISSUE-tkt-002) · sửa luôn engine lương đếm người phụ thuộc theo kỳ đăng ký (ISSUE-tkt-001, thuộc `du_lieu_tinh_luong`). Backend sửa cả 5 điểm: BUG-tkt-001 (in hoa `ma_nv` + kiểm nhân viên tồn tại ⇒ 404 `E-tkt-016`) · BUG-tkt-002 (trả NET với tỷ lệ khấu trừ 100% ⇒ 400 `E-tkt-004`; tỷ lệ 100% vẫn hợp lệ theo hợp đồng Mục 2.3) · BUG-tkt-003 · ISSUE-tkt-001 · ISSUE-tkt-002. Phase B lượt 2: **120/120 ca lá đạt, 0 lỗi, 13 bỏ qua có lý do**; `npm test` 1163 test, 1146 đạt, 4 lỗi = đúng 4 lỗi cũ (TC-hrm-301/316 và 2 nhóm cha); typecheck và eslint 0 lỗi. Đã cập nhật hợp đồng Mục 3.4 + Mục 7 (mã lỗi dòng 4b/5c chờ Architect xác nhận), SRS bảng trường `isResident`, data-model Mục 8, dev-notes Mục 1.12. ISSUE-tkt-004 của lượt 1 là kết luận sai của QA (HRM có quy ước in hoa `ma_nv`) — lỗi thật gộp vào BUG-tkt-001. **Còn mở:** ISSUE-tkt-003 (mở lại tháng xóa ghi đè chỉ tiêu, chờ BA/frontend) · ISSUE-tkt-005 (`npm audit`, task riêng). Commit `9fd34ec`.

**Code-reviewer `to_khai_thue` (2026-09-15):** ❌ Request changes — 11 finding, ghi ở `review-findings.md` mục "Review backend `be_maxv` — sub-cụm `to_khai_thue`". 🔴 **RVW-721:** kỳ lương vẫn mở lại được, rồi xóa được, sau khi Bảng tính thuế tháng đã chốt hoặc tờ khai quý đã xuất — số tờ khai lệch bảng lương mà không có tín hiệu nào; xóa kỳ cuốn theo snapshot thuế và các khoản ngoài lương. 🟡 RVW-722 đua ghi khoản ngoài lương với Chốt tháng · RVW-723 số tiền không có cận trên / không nguyên đồng ⇒ 500 · RVW-724 ngày không tồn tại ⇒ 500 hoặc lưu sai ngày · RVW-725 công ty cấp mới chưa có chính sách thuế ⇒ `E-tkt-015` · RVW-726 danh sách khoản ngoài lương lệch hợp đồng · RVW-727 chống trùng vãng lai chỉ theo họ tên. 🟢 RVW-728…731. **Bước tiếp theo:** chốt quy tắc RVW-721 và khóa định danh vãng lai RVW-727 → backend-engineer sửa → chạy lại Phase B → code-reviewer review lại.

**Sửa findings review + Phase B lượt 3 (2026-09-15, commit `aa1dd15`):** chủ dự án chốt RVW-721 "chặn, mở lại Bảng tính thuế trước" và RVW-727 "ưu tiên CCCD/MST"; phạm vi vòng này RVW-721…727 (🟢 RVW-728…731 để sau). Backend sửa cả 7: tháng còn khóa `TAX_SHEET` thì mở lại / xóa kỳ lương ⇒ 409 `E-dltl-029` (mã mới, chờ Architect) · dòng kỳ lương thành điểm phối hợp duy nhất — ghi khoản ngoài lương giữ `FOR SHARE`, chốt tháng / mở lại / xóa kỳ giữ `FOR UPDATE`, chốt tháng tính dòng trong giao dịch · trần số theo kiểu cột (`amount` nguyên đồng ≤ 999.999.999.999, trần/ngưỡng danh mục ≤ 9.999.999.999.999, ghi đè chỉ tiêu, chốt chặn cuối của engine) · ngày phải có thật · công ty chưa có chính sách thuế thì tự nạp bộ chuẩn · danh sách khoản ngoài lương mới nhất trước, kỳ không tồn tại ⇒ `E-tkt-017` · khóa vãng lai CCCD → MST → họ tên ở cả gộp dòng lẫn chỉ số chống trùng `hrm_oir_chong_trung_v2`. Phase B lượt 3: **130/130 ca lá đạt, 0 lỗi, 13 bỏ qua** (thêm 10 ca); `npm test` 1186 test, 1169 đạt, 4 lỗi = đúng 4 lỗi cũ; typecheck và eslint 0 lỗi. Đã cập nhật api-contract / data-model / SRS của hai sub-cụm, ADR-013, dev-notes, test-report, review-findings (FIXED từng mục). **Code-reviewer review lại (2026-09-15): ⚠️ Approve with comments** — xác nhận FIXED cả 7 (RVW-727 mới đạt ở mức mã, còn chờ áp index lên tenant thật), không còn 🔴. Finding mới: 🟡 RVW-732 (đổi công thức `recipientKey` chưa có bước chuyển tiếp — tháng chốt trước khi triển khai giữ khóa theo họ tên nên tờ khai quý chưa xuất có thể đếm trùng người ở [16]/[19]; `engineVersion` vẫn `v1`; chưa có câu quét trùng theo khóa v2) · 🟢 RVW-733 (DB giả truyền `db` làm `tx`) · 🟢 RVW-734 (ánh xạ seed chính sách thuế chép 3 nơi, thiếu ca `getTaxPolicies` bảng rỗng) · 🟢 RVW-735 (xóa kỳ lương DRAFT vẫn xóa theo khoản ngoài lương, không ghi nhật ký — cần BA quyết) · 🟢 RVW-736 (thứ tự dòng Bảng tính thuế lệch hợp đồng Mục 0.4). **Chủ dự án chốt (2026-09-15):** sửa phần mã RVW-732 ngay · làm RVW-734, RVW-736 · RVW-735 chặn xóa kỳ còn khoản thu nhập ngoài lương · RVW-733 để sau · commit một lần sau khi test và review lại. **Đã sửa (commit `aa1dd15`):** dòng chốt ghim `engineVersion = 'v2'`; biểu thức khóa vãng lai dùng chung `sqlKhoaVangLai` cho index và 2 mục rà soát mới (`khoan-ngoai-trung-v2`, `bang-thue-khoa-vang-lai-cu`); một hàm ánh xạ seed chính sách thuế `veDuLieuChinhSach`; xóa kỳ còn khoản ngoài lương ⇒ 409 `E-dltl-030`; dòng Bảng tính thuế sắp theo loại lao động → họ tên cho cả Nháp lẫn Đã chốt. Phase B lượt 4: **132/132 ca lá đạt, 0 lỗi, 13 bỏ qua**; `npm test` 1191 test, 1174 đạt, 4 lỗi = đúng 4 lỗi cũ; typecheck và eslint 0 lỗi. Trình tự bắt buộc trước khi áp index v2 lên tenant thật ghi ở ADR-013 "Sửa đổi 2026-09-15" mục 4 — **đã chạy 2026-09-16 trên localhost**. **Code-reviewer review lại lần 2 (2026-09-15): ⚠️ Approve with comments** — xác nhận FIXED RVW-732 (ở mức mã), RVW-734, RVW-735, RVW-736. Finding mới: 🟡 RVW-737 (trình tự chuyển tiếp ở ADR-013 mục 4 chưa nêu việc mở lại tháng xóa ghi đè chỉ tiêu, chốt lại tính lại cả nhân viên nội bộ theo hồ sơ hiện hành, bước dọn khoản trùng đặt sai thứ tự, chưa có nhánh cho khoản trùng trong quý đã xuất; câu quét khoản trùng thiếu tháng và cờ đã chốt) · 🟢 RVW-738 (câu quét khóa cũ báo sai mãi trên CSDL locale `C` với dòng chỉ có họ tên — lọc thêm dòng có CCCD/MST) · 🟢 RVW-739 (dùng `satisfies` để `tsc` bắt cột chính sách thuế quên ánh xạ). Reviewer: không chặn commit; nên xử lý RVW-737 và RVW-738 trước khi duyệt các bước trên tenant thật. **Đã chạy trên tenant thật `maxv_0106861880_app` (localhost) 2026-09-16:** `hrm:ra-soat` 1/1 tenant sạch, 0 dòng cần dọn, gồm cả 2 mục mới `khoan-ngoai-trung-v2` và `bang-thue-khoa-vang-lai-cu`, nên không phải mở lại hay chốt lại tháng nào; `hrm:constraints` áp 12 mục, 5 mục đã có sẵn, 0 lỗi; đã đọc `pg_indexes` xác nhận có `hrm_oir_chong_trung_v2` và không còn index v1. **Sửa tiếp 2026-09-16:** RVW-738 (câu quét khóa cũ chỉ soi dòng vãng lai có CCCD hoặc MST, ca kiểm khẳng định thêm số dòng) và RVW-739 (`satisfies` ép đủ cột khi ánh xạ chính sách thuế); `npm test` 1191 test, 1174 đạt, 4 lỗi cũ; typecheck và eslint 0 lỗi. **Dọn nốt review 2026-09-16:** FIXED RVW-737 (ADR-013 mục 4 viết lại thành trình tự theo từng tháng, thêm nhánh cho khoản trùng nằm trong quý đã xuất, câu quét khoản trùng trả kèm năm/tháng và 2 cờ trạng thái), RVW-733 (DB giả nay bắt được lệnh chạy ngoài giao dịch), RVW-731 (2 ca đua thứ tự tất định, đã chứng minh ghim `FOR SHARE`), RVW-730 (bỏ ép kiểu ở ranh giới với `du_lieu_tinh_luong`), RVW-729 (hằng khấu trừ mặc định chỉ còn một bản). `npm test` 1194 test, 1177 đạt, 4 lỗi cũ; typecheck và eslint 0 lỗi. **Còn lại:** RVW-728 chờ quyết định — thêm cột chụp thông tin người nộp thuế lúc xuất (Architect) và ngữ nghĩa ghi đè người ký khi đánh dấu đã nộp (BA).

**Quyết định chủ dự án 2026-09-15 (bước 6):** chỉ tiêu [32] = 0 như [24], [25] (theo nhãn mẫu là thuế trên phí bảo hiểm nhân thọ của DN bảo hiểm nước ngoài; kế toán ghi đè nếu có) · thêm endpoint thứ 23 `GET /05-kk-tncn/file` tải lại file tờ khai đã xuất. **Khoảng trống dữ liệu:** chưa có nơi lưu "cơ quan thuế quản lý" — ô này trên file tờ khai đang để trống. Phần đầu mẫu in theo tên trường, chưa đánh số [01]–[15] vì chưa đối chiếu được bản gốc Thông tư 89/2026/TT-BTC.
...
| **Bảng lương tổng hợp snapshot** | `hrm_payroll_sheet_lines` | `db_<MST>` | 18 cột đóng băng tại thời điểm `lock`, xóa-ghi-lại toàn bộ mỗi lần khóa sổ. `@@unique([periodId, ma_nv])`. Từ 2026-09-11 nút "Tính lương" (màn Chốt kỳ lương) cũng ghi đè bảng này khi kỳ còn mở — kết quả tạm, khóa sổ chụp đè lần cuối. |
| **Chốt số bảng kê** *(2026-09-11, xem Mục 22)* | `hrm_payroll_module_locks` | `db_<MST>` | Có dòng = bảng kê đã chốt số trong kỳ, mở chốt = xóa dòng. Enum `PayrollModuleCode` 12 bảng kê. `@@unique([periodId, module])`, cascade theo kỳ. 8 bảng kê có dữ liệu riêng của kỳ bị chặn ghi (`E-dltl-027`). |
...
* **Chốt kỳ lương (`/payroll-periods/:id/...`)** *(2026-09-11, xem Mục 22)*: `GET closing` (12 bảng kê + "Bảng lương a/b NV") · `GET activities` (lịch sử, đọc lại `syslog`) · `POST modules/:module/lock` · `POST modules/:module/unlock` (**chỉ ADMIN/OWNER**) · `POST modules/lock-all` · `POST calculate` (tính lương tạm). Đặc tả: `du_lieu_tinh_luong/api-contract-du-lieu-tinh-luong.md` Mục 9.
...
## 22. Màn "Chốt kỳ lương" — chốt số từng bảng kê (2026-09-11)

**Yêu cầu:** chủ dự án gửi ảnh màn tham chiếu — màn Chốt kỳ lương 12 bảng kê (Chấm công, Tăng ca, KPI, Thưởng, Bù trừ, Sản lượng, Lương phần trăm, Thu nhập ngoài bảng lương, Chuyên cần, Giảm trừ thuế TNCN, Hồ sơ lương, Khoản hỗ trợ) + Lịch sử hoạt động; ô chọn tháng/năm và nút "Chốt kỳ lương" dời ra góc phải thanh HRM.

**4 quyết định chủ dự án chốt (SRS Mục 16.2):** (1) chốt số chặn ghi 8 bảng kê dữ liệu riêng của kỳ, 4 bảng kê dùng chung chỉ xác nhận đã rà · (2) chốt: người có quyền lương, mở chốt: chỉ chủ tài khoản · (3) khóa sổ không bắt buộc chốt đủ 12/12, chỉ cảnh báo · (4) "Tính lương" chạy engine + lưu kết quả tạm để hiện "Bảng lương a/b NV".

| Tầng | Đã làm | Tài liệu |
|---|---|---|
| Nghiệp vụ | `BR-dltl-030…034`, `E-dltl-027/028`, `AC-dltl-29…36`, vòng đời trạng thái bảng kê | `du_lieu_tinh_luong/srs-du-lieu-tinh-luong.md` Mục 16 |
| Dữ liệu | Bảng `hrm_payroll_module_locks` + enum `PayrollModuleCode` (chỉ thêm) | `du_lieu_tinh_luong/data-model-du-lieu-tinh-luong.md` Mục 12 |
| API | 6 endpoint mới; 10 đường ghi `/payroll-data/*` thêm nhánh 403 `E-dltl-027` | `du_lieu_tinh_luong/api-contract-du-lieu-tinh-luong.md` Mục 9 |
| Giao diện | Góc chọn tháng + nút "Chốt kỳ lương T{m}/{y}" trên thanh HRM (`PayrollPeriodProvider` bọc ở `HrmPage`, chọn kỳ theo tháng); trang `/hrm/chot-ky-luong`; bỏ khối `KyLuongSelector` — các nút vòng đời kỳ chuyển vào header trang mới; 8 màn nhập liệu chuyển chỉ đọc khi bảng kê đã chốt | `architecture/dev-notes.md` Mục 2.16 · `docs/HDSD_hrm.md` |

**Bằng chứng:** `npm test` 871 ca — 867 pass, 4 fail (đúng 4 ca đỏ cố ý có sẵn TC-hrm-301/316 + 2 suite cha; baseline trước khi sửa 851 ca/5 fail) · 9 ca mới `hrmPayrollClosing.test.ts` pass · `typecheck`/`lint` BE sạch · FE `tsc -b`, eslint, `vite build` sạch · gọi thật service trên Postgres local: tổng quan + bộ lọc JSON `syslog` trả đúng dữ liệu kỳ cũ.

**Còn mở:** Tester-QA Phase B + Code Reviewer độc lập chưa chạy; giao diện chưa kiểm trên trình duyệt đã đăng nhập (cần người dùng đăng nhập — không tự nhập mật khẩu); production chưa `sync:tenants` (bắt buộc cùng lượt deploy, xem api-contract Mục 9.4).
...
| 2026-09-14 | architect (cùng phiên) | Đọc `BangLuongTable.tsx`/`cotBangLuong.ts`/`bangLuongExcel.ts`/`BangLuongPanel.tsx` thật, viết kế hoạch triển khai 6 task tại `docs/superpowers/plans/2026-09-14-hrm-ui-foundation-bang-luong-pilot.md`; phát hiện thêm: thêm cột Lương %/Chuyên cần kéo theo phải sửa `bangLuongExcel.ts` (bỏ sheet "Chi tiết thu nhập" dư thừa + dòng ghi chú sai) — đã đưa vào Task 2 của kế hoạch.
| 2026-09-14 | architect + subagent-driven-development (cùng phiên, trên nhánh `dev_fe`) | Triển khai đủ 6 task: cài `@mui/x-data-grid`/`react-hook-form`/`@hookform/resolvers`; thêm 2 cột Lương %/Chuyên cần + sửa Excel; dựng `AsyncState`/`HrmDataTable`/`HrmFormDialog` tại `features/hrm/_shared/components/`; chuyển `BangLuongTable.tsx` sang `HrmDataTable` (pilot). Mỗi task qua review riêng (1-2 vòng sửa mỗi task khi cần) + 1 review tổng toàn nhánh + 1 đợt sửa cuối. Phát hiện đáng chú ý giữa chừng: column pinning của `@mui/x-data-grid` bản Community thực ra KHÔNG tồn tại (chỉ có ở bản Pro trả phí, khác giả định ban đầu trong ADR-011) — đã thay bằng sticky CSS thủ công, xem `ADR-011` Nguồn/Consequences cập nhật qua ledger. Commit `dc403d6`..`ec3a2d8` (11 commit, xem `.superpowers/sdd/2026-09-14-hrm-ui-foundation-bang-luong-pilot/progress.md` để có đầy đủ chi tiết từng review/ruling). **Còn nợ:** màn Bảng lương CHƯA được click-test thật trên trình duyệt (không có tài khoản đăng nhập, DB dev chưa có kỳ lương mẫu) — cần chủ dự án tự mở `/hrm/bang-luong` kiểm tra trực quan (cột dính khi cuộn ngang, mật độ dòng theo 3 mức Compact/Standard/Comfortable, dòng Tổng cộng, xuất Excel) trước khi merge lên `main`. `HrmFormDialog` chưa có màn nào dùng — 3 vấn đề nhỏ đã biết trước (reset khi mở lại, chưa có prop `error`, chưa hoist invariant `width`/`minWidth` của cột dính vào `HrmDataTable`) cố ý để dành cho task pilot #2 sau này.
