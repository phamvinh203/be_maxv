...
| 7 | Dọn + nối route chính thức: 23 endpoint chạy từ file route đã commit, bỏ 8 route nháp, cất ~2.060 dòng mã nháp cũ vào git stash, bổ sung `dangApDung` cho danh sách chính sách thuế, test HTTP giả lập cho ma trận quyền | chưa commit |
...
**Còn chờ quyết định:** `GAP-QA-tkt-01` (khoản chịu thuế toàn phần trả NET — hiện đang chặn bằng E-tkt-004) · mở rộng BR-tkt-003 cho cả 4 nhóm · `OQ-tkt-03` · `OQ-tkt-05` · 2 lỗ hổng đặc tả phát hiện ở bước 5: người thời vụ/thử việc có khoản chịu thuế toàn phần, và người không có hợp đồng hiệu lực nhận thưởng — cả hai hiện KHÔNG phát sinh thuế trên khoản đó. **Ngoài phạm vi:** engine lương đếm người phụ thuộc không xét kỳ đăng ký (BUG-dltl-005) — đã tách task riêng. Cả 23 endpoint đã nối route và commit; **chưa có ca HTTP chạy trên DB thật** (mới có test HTTP giả lập cho nối route + ma trận quyền). Mã nháp cũ phía máy chủ nằm trong git stash có nhãn `to_khai_thue: ma nhap cu da viet lai o buoc 3-6` (không commit); giao diện nháp `hdđt_maxv` vẫn gọi các route cũ đã bỏ — sửa khi bật lại frontend. **Bước tiếp theo theo quy trình:** tester-qa Phase B (chạy test HTTP thật, xuất `qa/test-report.md` + `qa/issues-and-bugs.md`) rồi code-reviewer.
