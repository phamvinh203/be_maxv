...
test("deleteCompanySchema: cắt khoảng trắng thừa đầu/cuối", () => {
  assert.equal(deleteCompanySchema.parse({ maSoThue: `  ${MST} ` }).maSoThue, MST);
});

test("deleteCompanySchema: nhận MST chi nhánh 10+3 số", () => {
  assert.equal(deleteCompanySchema.parse({ maSoThue: BRANCH_MST }).maSoThue, BRANCH_MST);
});

test("deleteCompanySchema: khoảng trắng GIỮA chuỗi là định dạng rác -> chặn", () => {
  assert.equal(deleteCompanySchema.safeParse({ maSoThue: "0106 861880" }).success, false);
});
