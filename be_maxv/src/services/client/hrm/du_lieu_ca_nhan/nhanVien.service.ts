...
import type {
  ...
  NhanVienListQuery,
  NhanVienUpdateInput,
} from '../../../../validators/hrm/du_lieu_ca_nhan/nhanVien.validator';
...
export async function getNhanVien(
  ...
  const [nv, hopDongTheoMa] = await Promise.all([
    db.hrm_nhan_vien.findFirst({
      where: { ma_nv: maNv, da_xoa: false },
      select: nhanVienSelect,
    }),
    hopDongHienHanhTheoNv(db, [maNv]),
  ]);
  if (!nv) throw new NotFoundError(MESSAGES.HRM.NHAN_VIEN_NOT_FOUND);
...
export async function createNhanVien(
  ...
  const [, maNv] = await Promise.all([
    body.ma_pb ? assertPhongBanTonTai(db, body.ma_pb) : undefined,
    body.ma_nv ? body.ma_nv : sinhMaNhanVien(db),
  ]);
