...
export function HangHoaList(): JSX.Element {
  const { data, isLoading, isError, error, refetch } = useHangHoaList({ limit: 500 });
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%', bgcolor: 'background.default' }}>
      <CatalogToolbar
        ...
        actions={[
          { title: 'Sửa', icon: <EditIcon fontSize="small" />, disabled: !selected, onClick: () => selected && openForm('edit', selected.ma_vt) },
          { title: 'Copy', icon: <ContentCopyIcon fontSize="small" />, disabled: !selected, onClick: () => selected && openForm('copy', selected.ma_vt) },
          { title: 'Xem', icon: <VisibilityIcon fontSize="small" />, disabled: !selected, onClick: () => selected && openForm('view', selected.ma_vt) },
          { title: 'Đổi mã', icon: <SwapHorizIcon fontSize="small" />, disabled: !selected, onClick: () => selected && setDoiMa(selected.ma_vt) },
          { title: 'Xóa', icon: <DeleteIcon fontSize="small" />, disabled: !selected, color: 'error', onClick: () => setDeleteOpen(true) },
        ]}
        moreItems={[
          { icon: <GridOnIcon fontSize="small" />, label: 'Xuất Excel', onClick: noop },
          { icon: <UploadFileIcon fontSize="small" />, label: 'Lấy dữ liệu từ tệp…', onClick: noop },
          { icon: <FileDownloadIcon fontSize="small" />, label: 'Tải tệp mẫu…', onClick: noop },
          { icon: <LockIcon fontSize="small" />, label: 'Khóa cột', onClick: noop },
        ]}
      />
            ...
            {!isLoading && list.filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={18} align="center" sx={{ py: 6, color: 'text.secondary' }}>
                  {list.searchInput ? 'Không tìm thấy hàng hóa phù hợp' : 'Chưa có hàng hóa nào'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
